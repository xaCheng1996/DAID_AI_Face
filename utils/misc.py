...
def get_all_preds_labels_race(model, loader, device):
    ...
    intersect_gender_race_labels = {
        'male':{
            '1': torch.tensor([], dtype=torch.int32),
            '2': torch.tensor([], dtype=torch.int32),
            '3': torch.tensor([], dtype=torch.int32),
            '4': torch.tensor([], dtype=torch.int32),
            '5': torch.tensor([], dtype=torch.int32),
            '6': torch.tensor([], dtype=torch.int32),
            '7': torch.tensor([], dtype=torch.int32),
            '8': torch.tensor([], dtype=torch.int32),
            '9': torch.tensor([], dtype=torch.int32),
            '10': torch.tensor([], dtype=torch.int32),
        },
        'female':{
            '1': torch.tensor([], dtype=torch.int32),
            '2': torch.tensor([], dtype=torch.int32),
    ...
    cnt = 0
    with torch.no_grad():
        for images, labels, gender, race, path, ori_path in tqdm(loader, mininterval=10.0):
            # cnt += 1
            # if cnt >= 10:
            #     return all_labels.numpy(), all_preds.numpy(), all_gender, all_race, all_paths, all_ori_path
            images, labels = images.to(device), labels.to(device)
            # with autocast():
