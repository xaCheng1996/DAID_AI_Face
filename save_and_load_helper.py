...
def load(logger, model, config):
    logger.info(f"Loading best model from {os.path.join(config['data']['save_checkpoint'], 'checkpoint.pth.tar')}")
    state_dict = torch.load(os.path.join(config['data']['save_checkpoint'], "checkpoint.pth.tar"), weights_only=False)["state_dict"]
    ...
    if not flag:
        ...
        model.load_state_dict(
            torch.load(os.path.join(config['data']['save_checkpoint'], "checkpoint.pth.tar"), weights_only=False)["state_dict"])
    #
    # from collections import OrderedDict
    # new_state_dict = OrderedDict()
    # for k, v in state_dict.items():
